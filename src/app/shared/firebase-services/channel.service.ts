...
export class ChannelService {
  ...
  async deleteChannel(channel: Channel) {
    await deleteDoc(
      doc(collection(this.firestore, 'channels'), channel.id)
    ).catch((err) => {
      console.log(err);
    });
  }
  ...
  getChannels() {
    ...
    onSnapshot(q, (snapshot) => {
      const channels = snapshot.docs.map((doc) =>
        Channel.fromFirestore({ id: doc.id, data: () => doc.data() })
      );
  ...
  validateInputChannelName(
    ...
    if (newChannelName.length > 0) {
      if (newChannelName.length < 5) {
        channelNameError =
          'Der Channel-Name muss mindestens 5 Zeichen lang sein.';
      } else if (newChannelName.includes(' ')) {
        channelNameError = 'Der Channel-Name darf keine Leerzeichen enthalten.';
      } else if (
        !this.channelNameIsUnique(newChannelName, currentChannelName)
      ) {
    ...
    if (newChannelDescription.length > 0) {
      if (newChannelDescription.length < 5) {
        channelDescriptionError =
          'Die Beschreibung muss mindestens 5 Zeichen lang sein.';
      } else {
        channelDescriptionError = '';
      }
    } else {
  ...
  channelNameIsUnique(name: string, currentChannelName: string | ''): boolean {
    if (name == currentChannelName) {
      ...
      return !channels.some(
        (channel) => channel.name.toLowerCase() === channelNameLowerCase
      );
    }
  }
}
